...
url = """https://loft.com.br/venda/imoveis/sp/
sao-paulo/jardim-america_sao-paulo_sp"""
...
if st.button("Scrape Website"):
    ...
    for i in range(2, 50):
        contents = soup.find_all(
            "div",
            attrs={
                "class": "MuiGrid-root jss121 MuiGrid-item MuiGrid-grid-xs-12 MuiGrid-grid-sm-6 MuiGrid-grid-md-3 MuiGrid-grid-lg-3"
            },
        )
        for j, info in enumerate(contents):
            ...
            latitude = [text for text in soup.text.split(",") if "latitude" in text][
                0
            ].split('"latitude":')[-1]
            longitude = [text for text in soup.text.split(",") if "longitude" in text][
                0
            ].split('"longitude":')[-1]
            pattern = r"-\d+\.\d+"
    ...
    st.success(
        'File "houses.csv" is ready to be saved. Use the button on the sidebar to save file locally.'
    )
