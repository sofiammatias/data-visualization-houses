...
class ParquetDatasetPiece:
    ...
    def __init__(
        ...
        self._init(path, open_file_func, file_options, row_group, partition_keys)
    ...
    def _init(self, path, open_file_func, file_options, row_group, partition_keys):
    ...
    def __repr__(self):
        return "{}({!r}, row_group={!r}, partition_keys={!r})".format(
            type(self).__name__, self.path, self.row_group, self.partition_keys
        )
    ...
    def __str__(self):
        ...
        if len(self.partition_keys) > 0:
            partition_str = ", ".join(
                "{}={}".format(name, index) for name, index in self.partition_keys
            )
...
class ParquetPartitions:
    ...
    def equals(self, other):
        ...
        return (
            ...
            and self.partition_names == other.partition_names
        )
    ...
    def get_index(self, level, name, key):
        ...
        if level == len(self.levels):
            if name in self.partition_names:
                raise ValueError(
                    "{} was the name of the partition in " "another level".format(name)
                )
    ...
    def filter_accepts_partition(self, part_key, filter, level):
        ...
        if op in {"in", "not in"}:
            if not isinstance(f_value, Collection):
                raise TypeError("'%s' object is not a collection", f_type.__name__)
            if not f_value:
                raise ValueError("Cannot use empty collection as filter value")
            if len({type(item) for item in f_value}) != 1:
                raise ValueError(
                    "All elements of the collection '%s' must be" " of same type",
                    f_value,
                )
        ...
        p_value = f_type(self.levels[level].dictionary[p_value_index].as_py())
        ...
        if op == "=" or op == "==":
            ...
            raise ValueError("'%s' is not a valid operator in predicates.", filter[1])
...
class ParquetManifest:
    ...
    def _should_silently_exclude(self, file_name):
        return (
            ...
            or file_name in EXCLUDED_PARQUET_PATHS  # Hidden files starting with _
        )
    ...
    def _parse_partition(self, dirname):
        if self.partition_scheme == "hive":
            ...
            raise NotImplementedError(
                "partition schema: {}".format(self.partition_scheme)
            )

    def _push_pieces(self, files, part_keys):
        self.pieces.extend(
            [
                ParquetDatasetPiece._create(
                    path, partition_keys=part_keys, open_file_func=self.open_file_func
                )
                for path in files
            ]
        )
...
def _parse_hive_partition(value):
    if "=" not in value:
        raise ValueError(
            "Directory name did not appear to be a " "partition: {}".format(value)
        )
...
def _open_dataset_file(dataset, path, meta=None):
    if dataset.fs is not None and not isinstance(dataset.fs, legacyfs.LocalFileSystem):
...
""".format(
    ...
    def equals(self, other):
        ...
        for prop in ("memory_map", "buffer_size"):
            if getattr(self._ds_metadata, prop) != getattr(other._ds_metadata, prop):
                return False
    ...
    def _filter(self, filters):
        ...
        def one_filter_accepts(piece, filter):
            return all(
                accepts_filter(part_key, filter, level)
                for level, part_key in enumerate(piece.partition_keys)
            )

        def all_filters_accept(piece):
            return any(
                all(one_filter_accepts(piece, f) for f in conjunction)
                for conjunction in filters
            )
    ...
    _fs = property(operator.attrgetter("_ds_metadata.fs"))
    ...
    def metadata(self):
        ...
        """
        warnings.warn(
            _DEPR_MSG.format("ParquetDataset.metadata", ""), FutureWarning, stacklevel=2
        )
    ...
    _common_metadata = property(operator.attrgetter("_ds_metadata.common_metadata"))
    ...
    if _is_path_like(path_or_paths) and fs.isdir(path_or_paths):
        ...
        for path in path_or_paths:
            if not fs.isfile(path):
                raise OSError("Passed non-file path: {}".format(path))
            piece = ParquetDatasetPiece._create(path, open_file_func=open_file_func)
            pieces.append(piece)
...
class _ParquetDatasetV2:
    ...
    def pieces(self):
        warnings.warn(
            _DEPR_MSG.format(
                "ParquetDataset.pieces", " Use the '.fragments' attribute instead"
            ),
            FutureWarning,
            stacklevel=2,
        )
...
'delete_matching'
    ...
    **kwargs : dict,
    ...
    if partition_filename_cb is not None and basename_template is not None:
        raise ValueError(
            msg_confl.format(
                "basename_template", "partition_filename_cb", use_legacy_dataset
            )
        )

    if partition_cols is not None and partitioning is not None:
        raise ValueError(
            msg_confl.format("partitioning", "partition_cols", use_legacy_dataset)
        )

    metadata_collector = kwargs.pop("metadata_collector", None)
    if metadata_collector is not None and file_visitor is not None:
        raise ValueError(
            msg_confl.format("file_visitor", "metadata_collector", use_legacy_dataset)
        )
    ...
    if not use_legacy_dataset:
        ...
        msg = (
            "The '{}' argument is not supported with the new dataset " "implementation."
        )
    ...
    if partition_cols is not None and len(partition_cols) > 0:
        ...
        for keys, subgroup in data_df.groupby(partition_keys, observed=True):
            ...
            with fs.open(full_path, "wb") as f:
                write_table(
                    subtable, f, metadata_collector=metadata_collector, **kwargs
                )
            if metadata_collector is not None:
        ...
        with fs.open(full_path, "wb") as f:
            write_table(table, f, metadata_collector=metadata_collector, **kwargs)
        if metadata_collector is not None:
