...
class HadoopFileSystem(_hdfsio.HadoopFileSystem, FileSystem):
    ...
    def __init__(
        ...
        warnings.warn(
            _DEPR_MSG.format("hdfs.HadoopFileSystem", "2.0.0", "fs.HadoopFileSystem"),
            FutureWarning,
            stacklevel=2,
        )
        if driver == "libhdfs":
    ...
    def __reduce__(self):
        return (
            HadoopFileSystem,
            (self.host, self.port, self.user, self.kerb_ticket, self.extra_conf),
        )
...
def connect(host="default", port=0, user=None, kerb_ticket=None, extra_conf=None):
    """
    ...
    return _connect(
        host=host, port=port, user=user, kerb_ticket=kerb_ticket, extra_conf=extra_conf
    )


def _connect(host="default", port=0, user=None, kerb_ticket=None, extra_conf=None):
