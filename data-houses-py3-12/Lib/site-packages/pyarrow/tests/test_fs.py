...
@pytest.fixture(
    params=[
        ...
        pytest.param(
            pytest.lazy_fixture("s3fs"), id="S3FileSystem", marks=pytest.mark.s3
        ),
        pytest.param(
            pytest.lazy_fixture("gcsfs"), id="GcsFileSystem", marks=pytest.mark.gcs
        ),
        pytest.param(
            pytest.lazy_fixture("hdfs"), id="HadoopFileSystem", marks=pytest.mark.hdfs
        ),
...
def test_s3_proxy_options(monkeypatch, pickle_module):
    ...
    with pytest.raises(pa.ArrowInvalid):
        S3FileSystem(
            proxy_options={"scheme": "htttp", "host": "localhost", "port": 8999}
        )
...
def test_s3_real_aws_region_selection():
    ...
    fs, path = FileSystem.from_uri("s3://x-arrow-non-existent-bucket?region=us-east-3")
