...
from pyarrow.tests.parquet.common import _check_roundtrip, parametrize_legacy_dataset
...
def test_decimal_roundtrip(tempdir, use_legacy_dataset):
    ...
    result_table = _read_table(string_filename, use_legacy_dataset=use_legacy_dataset)
...
def test_nested_list_nonnullable_roundtrip_bug(use_legacy_dataset):
    ...
    _check_roundtrip(t, data_page_size=4096, use_legacy_dataset=use_legacy_dataset)


@parametrize_legacy_dataset
def test_nested_list_struct_multiple_batches_roundtrip(tempdir, use_legacy_dataset):
    # Reproduce failure in ARROW-11024
    data = [[{"x": "abc", "y": "abc"}]] * 100 + [[{"x": "abc", "y": "gcb"}]] * 100
    table = pa.table([pa.array(data)], names=["column"])
    _check_roundtrip(table, row_group_size=20, use_legacy_dataset=use_legacy_dataset)
    ...
    _check_roundtrip(table, row_group_size=10, use_legacy_dataset=use_legacy_dataset)
...
def test_large_list_records():
    ...
    list_values = [
        list(map(int, np.random.randint(0, 100, size=x))) if i % 8 else None
...
def test_parquet_nested_convenience(tempdir, use_legacy_dataset):
    ...
    read = pq.read_table(path, columns=["a"], use_legacy_dataset=use_legacy_dataset)
...
def test_binary_array_overflow_to_chunked(use_legacy_dataset):
    ...
    read_tbl = _simple_table_roundtrip(tbl, use_legacy_dataset=use_legacy_dataset)
...
def test_list_of_binary_large_cell(use_legacy_dataset):
    ...
    read_table = _simple_table_roundtrip(table, use_legacy_dataset=use_legacy_dataset)
