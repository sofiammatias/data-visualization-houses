...
def test_pandas_parquet_column_multiindex(tempdir, use_legacy_dataset):
    ...
    table_read = pq.read_pandas(filename, use_legacy_dataset=use_legacy_dataset)
...
def test_pandas_column_selection(tempdir, use_legacy_dataset):
    ...
    # being read uniquely.
    table_read = _read_table(
        filename, columns=["uint8", "uint8"], use_legacy_dataset=use_legacy_dataset
    )
    df_read = table_read.to_pandas()
...
def test_pandas_parquet_native_file_roundtrip(tempdir, use_legacy_dataset):
    ...
    df_read = _read_table(reader, use_legacy_dataset=use_legacy_dataset).to_pandas()
    tm.assert_frame_equal(df, df_read)
...
def test_read_pandas_column_subset(tempdir, use_legacy_dataset):
    ...
    df_read = pq.read_pandas(
        reader, columns=["strings", "uint8"], use_legacy_dataset=use_legacy_dataset
    ).to_pandas()
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_backwards_compatible_index_multi_level_named(datadir, use_legacy_dataset):
    expected_string = b"""\
carat        cut  color  clarity  depth  table  price     x     y     z
 0.23      Ideal      E      SI2   61.5   55.0    326  3.95  3.98  2.43
 0.21    Premium      E      SI1   59.8   61.0    326  3.89  3.84  2.31
 ...
 0.23  Very Good      H      VS1   59.4   61.0    338  4.00  4.05  2.39"""
    ...
    table = _read_table(
        datadir / "v0.7.1.all-named-index.parquet",
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_backwards_compatible_index_multi_level_some_named(datadir, use_legacy_dataset):
    expected_string = b"""\
carat        cut  color  clarity  depth  table  price     x     y     z
 0.23      Ideal      E      SI2   61.5   55.0    326  3.95  3.98  2.43
 0.21    Premium      E      SI1   59.8   61.0    326  3.89  3.84  2.31
 ...
 0.23  Very Good      H      VS1   59.4   61.0    338  4.00  4.05  2.39"""
    ...
    table = _read_table(
        datadir / "v0.7.1.some-named-index.parquet",
        use_legacy_dataset=use_legacy_dataset,
    )
    result = table.to_pandas()
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_backwards_compatible_column_metadata_handling(datadir, use_legacy_dataset):
    ...
    table = _read_table(path, columns=["a"], use_legacy_dataset=use_legacy_dataset)
    result = table.to_pandas()
...
def test_categorical_index_survives_roundtrip(use_legacy_dataset):
    ...
    ref_df = pq.read_pandas(
        bos.getvalue(), use_legacy_dataset=use_legacy_dataset
    ).to_pandas()
...
def test_categorical_order_survives_roundtrip(use_legacy_dataset):
    ...
    result = pq.read_pandas(contents, use_legacy_dataset=use_legacy_dataset).to_pandas()
...
def test_pandas_categorical_na_type_row_groups(use_legacy_dataset):
    ...
    result = pq.read_table(buf.getvalue(), use_legacy_dataset=use_legacy_dataset)
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_write_to_dataset_pandas_preserve_extensiondtypes(tempdir, use_legacy_dataset):
