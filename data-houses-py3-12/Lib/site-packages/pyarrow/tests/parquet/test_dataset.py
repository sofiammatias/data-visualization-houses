...
def test_filesystem_uri(tempdir, use_legacy_dataset):
    ...
    result = pq.read_table(
        path, filesystem=fs.LocalFileSystem(), use_legacy_dataset=use_legacy_dataset
    )
    assert result.equals(table)
...
def test_create_parquet_dataset_multi_threaded(tempdir):
    ...
    manifest = pq.ParquetManifest(base_path, filesystem=fs, metadata_nthreads=1)
    with pytest.warns(FutureWarning, match="Specifying the 'metadata_nthreads'"):
        dataset = pq.ParquetDataset(
            base_path, filesystem=fs, metadata_nthreads=16, use_legacy_dataset=True
        )
...
def test_read_partitioned_columns_selection(tempdir, use_legacy_dataset):
    ...
    dataset = pq.ParquetDataset(base_path, use_legacy_dataset=use_legacy_dataset)
...
def test_filters_equivalency(tempdir, use_legacy_dataset):
    ...
    dataset = pq.ParquetDataset(
        base_path, filesystem=fs, filters=filters, use_legacy_dataset=use_legacy_dataset
    )
    table = dataset.read()
    result_df = table.to_pandas().reset_index(drop=True)
    ...
    if use_legacy_dataset:
        ...
        with pytest.raises(NotImplementedError):
            ...
            pq.ParquetDataset(
                base_path, filesystem=fs, filters=filters, use_legacy_dataset=True
            )
        with pytest.raises(NotImplementedError):
            filters = [[("string", "==", "1\0a")]]
            pq.ParquetDataset(
        ...
        for filters in [[[("string", "==", b"1\0a")]], [[("string", "==", "1\0a")]]]:
            dataset = pq.ParquetDataset(
                base_path, filesystem=fs, filters=filters, use_legacy_dataset=False
            )
...
def test_filters_read_table(tempdir, use_legacy_dataset, filters, read_method):
    ...
    kwargs = dict(filesystem=fs, filters=filters, use_legacy_dataset=use_legacy_dataset)
...
def test_read_s3fs(s3_example_s3fs, use_legacy_dataset):
    ...
    result = _read_table(path, filesystem=fs, use_legacy_dataset=use_legacy_dataset)
    assert result.equals(table)
...
@pytest.mark.s3
@parametrize_legacy_dataset
def test_read_partitioned_directory_s3fs_wrapper(s3_example_s3fs, use_legacy_dataset):
    import s3fs
...
def test_read_partitioned_directory_s3fs(s3_example_s3fs, use_legacy_dataset):
    fs, path = s3_example_s3fs
    _partition_test_for_filesystem(fs, path, use_legacy_dataset=use_legacy_dataset)
...
def _test_read_common_metadata_files(fs, base_path):
    ...
    dataset = pq.ParquetDataset(base_path, filesystem=fs, use_legacy_dataset=True)
    with pytest.warns(FutureWarning):
    ...
    dataset2 = pq.ParquetDataset([base_path], filesystem=fs, use_legacy_dataset=True)
...
def test_read_metadata_files(tempdir):
    ...
    dataset = pq.ParquetDataset(tempdir, filesystem=fs, use_legacy_dataset=True)
    with pytest.warns(FutureWarning):
...
def test_filter_before_validate_schema(tempdir, use_legacy_dataset):
    ...
    table = pq.read_table(
        tempdir, filters=[[("A", "==", 0)]], use_legacy_dataset=use_legacy_dataset
    )
    assert table.column("B").equals(pa.chunked_array([[1, 2, 3]]))


@pytest.mark.pandas
@pytest.mark.filterwarnings("ignore:Specifying the 'metadata':FutureWarning")
@parametrize_legacy_dataset
def test_read_multiple_files(tempdir, use_legacy_dataset):
    ...
    if use_legacy_dataset:
        ...
        with pytest.warns(FutureWarning, match="Specifying the 'schema'"):
            result3 = pq.ParquetDataset(
                dirpath, schema=metadata.schema, use_legacy_dataset=True
            ).read()
    ...
    pq.read_table(dirpath, use_threads=True, use_legacy_dataset=use_legacy_dataset)
...
def test_dataset_enable_buffered_stream(tempdir, use_legacy_dataset):
    ...
    with pytest.raises(ValueError):
        pq.ParquetDataset(
            dirpath, buffer_size=-64, use_legacy_dataset=use_legacy_dataset
        )

    for buffer_size in [128, 1024]:
        dataset = pq.ParquetDataset(
            dirpath, buffer_size=buffer_size, use_legacy_dataset=use_legacy_dataset
        )
        assert dataset.read().equals(table)
...
def test_dataset_enable_pre_buffer(tempdir, use_legacy_dataset):
    ...
    for pre_buffer in (True, False):
        dataset = pq.ParquetDataset(
            dirpath, pre_buffer=pre_buffer, use_legacy_dataset=use_legacy_dataset
        )
        assert dataset.read().equals(table)
...
def test_ignore_custom_prefixes(tempdir, use_legacy_dataset):
    ...
    read = pq.read_table(
        tempdir, use_legacy_dataset=use_legacy_dataset, ignore_prefixes=["_private"]
    )
...
def test_empty_directory(tempdir, use_legacy_dataset):
    ...
    dataset = pq.ParquetDataset(empty_dir, use_legacy_dataset=use_legacy_dataset)
    result = dataset.read()
...
def _test_write_to_dataset_with_partitions(
    base_path, use_legacy_dataset=True, filesystem=None, schema=None, index_name=None
):
    import pandas as pd
    import pandas.testing as tm
...
def _test_write_to_dataset_no_partitions(
    base_path, use_legacy_dataset=True, filesystem=None
):
    import pandas as pd
    ...
    output_files = [
        file for file in filesystem.ls(str(base_path)) if file.endswith(".parquet")
    ]
    ...
    input_table = pq.ParquetDataset(
        base_path, filesystem=filesystem, use_legacy_dataset=use_legacy_dataset
    ).read()
    input_df = input_table.to_pandas()
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_write_to_dataset_with_partitions_and_schema(tempdir, use_legacy_dataset):
    schema = pa.schema(
        [
...
@pytest.mark.pandas
@parametrize_legacy_dataset
def test_write_to_dataset_with_partitions_and_index_name(tempdir, use_legacy_dataset):
    _test_write_to_dataset_with_partitions(
...
def test_write_to_dataset_pathlib(tempdir, use_legacy_dataset):
    _test_write_to_dataset_with_partitions(tempdir / "test1", use_legacy_dataset)
    _test_write_to_dataset_no_partitions(tempdir / "test2", use_legacy_dataset)
...
@pytest.mark.s3
@parametrize_legacy_dataset
def test_write_to_dataset_with_partitions_s3fs(s3_example_s3fs, use_legacy_dataset):
    fs, path = s3_example_s3fs

    _test_write_to_dataset_with_partitions(path, use_legacy_dataset, filesystem=fs)


@pytest.mark.pandas
@pytest.mark.s3
@parametrize_legacy_dataset
def test_write_to_dataset_no_partitions_s3fs(s3_example_s3fs, use_legacy_dataset):
    fs, path = s3_example_s3fs

    _test_write_to_dataset_no_partitions(path, use_legacy_dataset, filesystem=fs)
    ...
    output_df = pd.DataFrame(
        {
            ...
            "num": list(range(10)),
            "nan": [np.nan] * 10,
            "date": np.arange("2017-01-01", "2017-01-11", dtype="datetime64[D]"),
        }
    )
    partition_by = ["group1", "group2"]
    output_table = pa.Table.from_pandas(output_df)
...
def test_partitioned_dataset(tempdir, use_legacy_dataset):
    ...
    table = pq.ParquetDataset(path, use_legacy_dataset=use_legacy_dataset).read()
...
def test_dataset_read_dictionary(tempdir, use_legacy_dataset):
    ...
    pq.write_to_dataset(t1, root_path=str(path), use_legacy_dataset=use_legacy_dataset)
    pq.write_to_dataset(t2, root_path=str(path), use_legacy_dataset=use_legacy_dataset)

    result = pq.ParquetDataset(
        path, read_dictionary=["f0"], use_legacy_dataset=use_legacy_dataset
    ).read()
...
def test_read_table_schema(tempdir):
    ...
    with pytest.raises(ValueError, match="The 'schema' argument is only supported"):
        pq.read_table(tempdir / "data.parquet", schema=schema, use_legacy_dataset=True)

    # using ParquetDataset directory with non-legacy implementation
    result = pq.ParquetDataset(tempdir, schema=schema, use_legacy_dataset=False)
    expected = pa.table({"a": [1, 2, 3, 1, 2, 3]}, schema=schema)
...
def test_dataset_partitioning(tempdir):
    ...
    result = pq.read_table(str(root_path), partitioning=part, use_legacy_dataset=False)
    ...
    with pytest.raises(ValueError):
        pq.read_table(str(root_path), partitioning=part, use_legacy_dataset=True)

    with pytest.raises(ValueError):
        pq.ParquetDataset(str(root_path), partitioning=part, use_legacy_dataset=True)
...
@pytest.mark.filterwarnings("ignore:'ParquetDataset:FutureWarning")
@parametrize_legacy_dataset
def test_parquet_dataset_partitions_piece_path_with_fsspec(tempdir, use_legacy_dataset):
...
def test_parquet_dataset_deprecated_properties(tempdir):
    ...
    with pytest.warns(FutureWarning, match="'ParquetDataset.common_metadata_path"):
...
def test_parquet_write_to_dataset_deprecated_properties(tempdir):
    ...
    with pytest.warns(FutureWarning, match="Passing 'use_legacy_dataset=True'"):
        pq.write_to_dataset(
            table, path, partition_filename_cb=lambda x: "filename.parquet"
        )
...
def test_parquet_write_to_dataset_unsupported_keywards_in_legacy(tempdir):
    ...
    with pytest.raises(ValueError, match="schema"):
        pq.write_to_dataset(
            table, path, use_legacy_dataset=True, schema=pa.schema([("a", pa.int32())])
        )

    with pytest.raises(ValueError, match="partitioning"):
        pq.write_to_dataset(table, path, use_legacy_dataset=True, partitioning=["a"])

    with pytest.raises(ValueError, match="use_threads"):
        pq.write_to_dataset(table, path, use_legacy_dataset=True, use_threads=False)

    with pytest.raises(ValueError, match="file_visitor"):
        pq.write_to_dataset(
            table, path, use_legacy_dataset=True, file_visitor=lambda x: x
        )

    with pytest.raises(ValueError, match="existing_data_behavior"):
        pq.write_to_dataset(
            table, path, use_legacy_dataset=True, existing_data_behavior="error"
        )

    with pytest.raises(ValueError, match="basename_template"):
        pq.write_to_dataset(
            table, path, use_legacy_dataset=True, basename_template="part-{i}.parquet"
        )
...
def test_write_to_dataset_conflicting_keywords(tempdir):
    ...
    with pytest.raises(
        ValueError,
        match="'basename_template' argument "
    ...
    with pytest.raises(
        ValueError,
        match="'partition_filename_cb' argument "
    ...
    with pytest.raises(
        ValueError,
        match="'partitioning' argument " "is not supported by use_legacy_dataset=True",
    ):
        pq.write_to_dataset(
            table,
            path,
    ...
    with pytest.raises(
        ValueError,
        match="'file_visitor' argument " "is not supported by use_legacy_dataset=True",
    ):
        pq.write_to_dataset(
            table,
            path,
    ...
    with pytest.raises(
        ValueError,
        match="'metadata_collector' argument "
        "is not supported by use_legacy_dataset=False",
    ):
        pq.write_to_dataset(
            table,
