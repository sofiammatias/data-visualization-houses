...
def test_set_data_page_size(use_legacy_dataset):
    ...
    for target_page_size in page_sizes:
        _check_roundtrip(
            t, data_page_size=target_page_size, use_legacy_dataset=use_legacy_dataset
        )
...
def test_memory_map(tempdir, use_legacy_dataset):
    ...
    _check_roundtrip(
        table,
        read_table_kwargs={"memory_map": True},
        version="2.6",
        use_legacy_dataset=use_legacy_dataset,
    )
    ...
    table_read = pq.read_pandas(
        filename, memory_map=True, use_legacy_dataset=use_legacy_dataset
    )
...
def test_enable_buffered_stream(tempdir, use_legacy_dataset):
    ...
    _check_roundtrip(
        table,
        read_table_kwargs={"buffer_size": 1025},
        version="2.6",
        use_legacy_dataset=use_legacy_dataset,
    )
    ...
    with open(filename, "wb") as f:
        _write_table(table, f, version="2.6")
    table_read = pq.read_pandas(
        filename, buffer_size=4096, use_legacy_dataset=use_legacy_dataset
    )
    assert table_read.equals(table)
...
def test_empty_table_roundtrip(use_legacy_dataset):
    ...
    _check_roundtrip(table, version="2.6", use_legacy_dataset=use_legacy_dataset)
...
def test_fspath(tempdir, use_legacy_dataset):
    ...
    result = _read_table(fs_protocol_obj, use_legacy_dataset=use_legacy_dataset)
...
def test_relative_paths(tempdir, use_legacy_dataset, filesystem, name):
    ...
    with util.change_cwd(tempdir):
        result = pq.read_table(
            name, filesystem=filesystem, use_legacy_dataset=use_legacy_dataset
        )
    assert result.equals(table)
...
def test_parquet_read_from_buffer(tempdir, use_legacy_dataset):
    ...
    with open(str(tempdir / "data.parquet"), "rb") as f:
        result = pq.read_table(pa.PythonFile(f), use_legacy_dataset=use_legacy_dataset)
    assert result.equals(table)
...
def test_column_encoding(use_legacy_dataset):
    ...
    _check_roundtrip(
        ...
        expected=mixed_table,
        use_dictionary=False,
        column_encoding={"a": "BYTE_STREAM_SPLIT", "b": "PLAIN", "c": "PLAIN"},
        use_legacy_dataset=use_legacy_dataset,
    )
    ...
    with pytest.raises(
        IOError, match="BYTE_STREAM_SPLIT only supports FLOAT and" " DOUBLE"
    ):
        _check_roundtrip(
            mixed_table,
            expected=mixed_table,
            use_dictionary=False,
            column_encoding={"a": "PLAIN", "b": "BYTE_STREAM_SPLIT", "c": "PLAIN"},
            use_legacy_dataset=use_legacy_dataset,
        )
...
def test_multithreaded_read(use_legacy_dataset):
    ...
    buf.seek(0)
    table1 = _read_table(buf, use_threads=True, use_legacy_dataset=use_legacy_dataset)

    buf.seek(0)
    table2 = _read_table(buf, use_threads=False, use_legacy_dataset=use_legacy_dataset)
