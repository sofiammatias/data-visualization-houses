...
@pytest.mark.pandas
@pytest.mark.parquet
@pytest.mark.filterwarnings("ignore:Passing 'use_legacy_dataset=True':FutureWarning")
...
def test_legacy_write_to_dataset_drops_null(tempdir):
    ...
    table = pa.table(
        {"part": ["a", "a", None, None], "col": list(range(4))}, schema=schema
    )
...
def test_write_iterable(tempdir):
    ...
    ds.write_dataset(
        reader, base_dir, basename_template="dat_{i}.arrow", format="feather"
    )
    result = ds.dataset(base_dir, format="ipc").to_table()
    assert result.equals(table)
